   ...
   // 機能レベル
	...
	for (UINT driverTypeIndex = 0; driverTypeIndex < numDriverTypes; driverTypeIndex++)
	{
		...
		hr = D3D11CreateDeviceAndSwapChain(NULL,				// ディスプレイデバイスのアダプタ（ＮＵＬＬの場合最初に見つかったアダプタ）
										   ...
										   D3D11_SDK_VERSION,	// 
										   &sd,				// スワップチェインの設定
										   &mpSwapChain,		// IDXGIDwapChainインタフェース	
										   &mpDevice,		// ID3D11Deviceインタフェース
										   &mFeatureLevel,	// サポートされている機能レベル
										   &mpImmediateContext);	// デバイスコンテキスト
		if (SUCCEEDED(hr))
			break;
	}
	if (FAILED(hr))
	...
	hr = mpDevice->CreateRenderTargetView(pBackBuffer, NULL, &mpRenderTargetView);				//  ts
	pBackBuffer->Release();
	pBackBuffer = 0;
	if (FAILED(hr))
		return false;
	...
	hr = mpDevice->CreateTexture2D(&depthBufferDesc, NULL, &mDepthStencilBuffer);
	if (FAILED(hr))
	{
		return false;
	}
	...
	hr = mpDevice->CreateDepthStencilState(&depthStencilDesc, &mDepthStencilState);
	if (FAILED(hr))
	{
		return false;
	}
	...
	// デバイスコンテキストへセット
	mpImmediateContext->OMSetDepthStencilState(mDepthStencilState.Get(), 1);
	...
	hr = mpDevice->CreateDepthStencilView(mDepthStencilBuffer.Get(), &depthStencilViewDesc, &mDepthStencilView);
	if (FAILED(hr))
	{
		return false;
	}
	...
	mpImmediateContext->OMSetRenderTargets(1, mpRenderTargetView.GetAddressOf(), mDepthStencilView.Get());
	...
	hr = mpDevice->CreateRasterizerState(&rasterDesc, &mRasterState);
	if (FAILED(hr))
	{
		return false;
	}
	...
	mpImmediateContext->RSSetState(mRasterState.Get());
