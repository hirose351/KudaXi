...
void transScreenToWorld(XMFLOAT3 * out, int sx, int sy, float z)
{
	...
	XMFLOAT4X4 invMat, inv_proj, inv_view, ans;
	DX11MtxInverse(invMat, vpMat);
	DX11MtxInverse(inv_proj, proj);
	DX11MtxInverse(inv_view, view);

	DX11MtxMultiply(ans, inv_proj, inv_view);
...
void MtxAddPos(XMFLOAT4X4& _mtx, XMFLOAT3 _pos)
{
	_mtx._41 += _pos.x;
	_mtx._42 += _pos.y;
	_mtx._43 += _pos.z;
...
bool floatcheck(float checvalue, float value, float minimum) {
	...
	if (fabsf(checvalue - value) < minimum)
	{
		return true;
	}
	return false;
